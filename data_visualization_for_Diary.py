...
curs = conn.cursor()
idcount=1
sql0="select userId from DiaryView;"
curs.execute(sql0)
...
for row in rows:
    ...
    try:
        ...
        continue
    sql2 = str("select content as dcontent,savedDate as dsavedate from DiaryView where userId=%s;" %row[0])
    curs.execute(sql2)
...
days=['30m','1h','2h','3h','1d','2d','3d','4d','5d','after']
...
plt.ylabel("diary count")
plt.legend()
plt.title("diary count analysis")
plt.xticks(x,days)
plt.show()
...
#plt.xlabel("diarycount")
#plt.bar(xx,yy2,label='after join',alpha=0.4,color='r')


#plt.ylabel("relations count")
#plt.legend()
plt.title("diarycount analysis")
...
plt.pie(yy2, labels=counts, shadow=False, startangle=90)
plt.show()"""
